...
if [[ ${O_VERSION} -gt 9 ]]; then
    O_SERVER="odoo-bin"
    O_NAME="odoo"
else
    O_SERVER="openerp-server"
    O_NAME="openerp"
fi
YN='y'
...
while true; do
	...
	case ${YN} in
	[Yy]* )
	    sudo rm -r ${O_DIR}/.venv  || true
		if [[ ${O_VERSION} -lt 11 ]]; then V=''; else V='3'; fi
		echo -e "\e[96m\n- Installing odoo Requirements for python${V}\e[0m"
		sudo apt-get install python${V}-pip virtualenv libsasl2-dev libldap2-dev \
				     		 libxml2-dev python${V}-dev build-essential libssl-dev pylint \
...
sudo su ${O_USER} << EOF
		...
		if [ $? -ne 0 ]; then
			...
			pip install passlib python-dateutil psycopg2 pyyaml Pillow reportlab Python-Chart psutil requests werkzeug==0.16.0
...
sudo su ${O_USER} << EOF
	...
	ln -s ${O_DIR}/addons/* ${O_DIR}/.venv/lib/${PYTHON}/site-packages/odoo/addons 2> /dev/null
	ln -s ${O_DIR}/odoo/addons/* ${O_DIR}/.venv/lib/${PYTHON}/site-packages/odoo/addons 2> /dev/null
	ln -s ${O_DIR}/odoo/* ${O_DIR}/.venv/lib/${PYTHON}/site-packages/odoo/ 2> /dev/null
EOF
...
sudo  su - ${O_USER} -c "touch ${O_CONF}/odoo${O_VERSION}.cfg ${O_LOG}/odoo${O_VERSION}.log"

sudo cat <<EOF > ${O_CONF}/odoo${O_VERSION}.cfg
[options]
; This is the password that allows database operations:
addons_path = ${O_DIR}/addons,${O_DIR}/${O_NAME}/addons,${O_DIR}/addons_custom
admin_passwd = admin
db_user = ${O_DB_USER}
...
ln -s ${O_CONF}/* ${O_HOME}/config/  2> /dev/null
...
if [[ \$@  =~ "scaffold" ]]; then
	...
	${O_DIR}/${O_SERVER} -c ${O_CONF}/odoo${O_VERSION}.cfg \$@
fi
EOF
...
echo -e "\e[93m******************** Odoo${O_VERSION} installed successfully ********************"
